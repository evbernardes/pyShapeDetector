...
class Cone(Primitive):
    ...
    def __init__(self, model, decimals=None):
        ...
        if not (0 <= self.half_angle < np.pi / 2):
            raise ValueError(
                "half_angle must be between 0 and 90 degrees, "
    ...
    def fit(points, normals=None):
        ...
        if num_points < 6:
            raise ValueError("A minimun of 6 points are needed to fit a cone")

        if normals is None:
            raise NotImplementedError(
                "Fitting of cone without normals has not been implemented."
            )
    ...
    def align(self, axis):
        ...
        return Cone.from_appex_vector_half_angle(self.appex, vector, self.half_angle)
