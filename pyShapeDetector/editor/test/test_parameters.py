...
from pyShapeDetector.editor.parameter import ParameterBase, ParameterNumeric
...
def test_param_numeric():
    for type_ in (int, float):
        # Correct with limits
        good_descriptor = {"type": type_, "default": 0}
        ...
        good_descriptor = {"type": type_, "default": 0, "limits": [0, 2]}
        parameter = ParameterBase.create_from_dict("param_name", good_descriptor)
        extension = {
            "function": lambda var: [],
            "inputs": None,
        ...
        with pytest.raises(TypeError, match="No limits or default value"):
            bad_descriptor = {"type": type_}
            ParameterBase.create_from_dict("radius", bad_descriptor)

        with pytest.raises(TypeError, match="should be list or tuple of 2 elements"):
            bad_descriptor = {"type": type_, "limits": (1, 2, 3)}
            ParameterBase.create_from_dict("var", bad_descriptor)

        with pytest.raises(TypeError, match="Expected boolean for 'use_slider'"):
            bad_descriptor = {"type": type_, "limits": (1, 2), "use_slider": "asd"}
            ParameterBase.create_from_dict("var", bad_descriptor)
