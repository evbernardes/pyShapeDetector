...
class MultiDetector:
    ...
    def pcds_inliers(self):
        if not self._finished:
            raise RuntimeError(
                "MultiDetector still did not fit, try to run, " "see: MultiDetector.run"
            )
    ...
    def run(self, debug, compare_metric, metric_min, normals_reestimate, fuse_shapes):
        ...
        for idx in range(self.n_pcds):
            ...
            while (
                len(pcd_.points) > self.points_min
                and iteration < self.shapes_per_cluster
            ):
                if debug:
                    print(
                ...
                for detector in self.detectors:
                    # start = time.time()
                    shape, inliers, metrics = detector.fit(pcd_, debug=debug_detectors)
                ...
                if metric_min is not None and max(compare) < metric_min:
                    if debug:
                        print(
                            f"{compare_metric} of {max(compare)} is too "
                            "small, breaking..."
                        )
                    continue
                ...
                if debug:
                    print(
                        f"-> {shape.name.capitalize()} found, with "
