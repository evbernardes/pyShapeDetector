...
class PrimitiveLimits:
        ...
        if args is None:
            ...
            for arg in args:
                if len(arg) == 3:
                    ...
                    raise ValueError(
                        "Limits are defined by 2 or 3 attributes: "
                ...
                if isinstance(bounds, list) and len(bounds) == 2:
                    ...
                    raise ValueError(
                        "limits must be a list or tuple of 2 " f"elements, got {bounds}"
                    )
                ...
                limits_dict = {"func": func, "attribute": attribute, "bounds": bounds}
    ...
    def __add__(self, limits_other):
        ...
        if not isinstance(limits_other, PrimitiveLimits):
            raise TypeError(
                f"unsupported operand type(s) for +: PrimitiveLimits and '{type(limits_other)}'"
            )
